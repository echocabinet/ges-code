...
# ucm_add_dir_impl
macro(ucm_add_dir_impl result rec noh trim dirs_in)
    set(dirs "${dirs_in}")
    ...
    if(${rec})
        foreach(cur_dir ${dirs})
            ...
            foreach(subdir ${subdirs})
                ucm_add_dir_impl(${result} ${rec} ${noh} ${trim} "${cur_dir}/${subdir}")
            endforeach()
        endforeach()
    endif()
endmacro()
...
macro(ucm_add_dirs)
    cmake_parse_arguments(ARG "RECURSIVE;NO_HEADERS" "TO;FILTER_POP" "" ${ARGN})
    ...
    ucm_add_dir_impl(${ARG_TO} ${ARG_RECURSIVE} ${ARG_NO_HEADERS} ${ARG_FILTER_POP} "${ARG_UNPARSED_ARGUMENTS}")
endmacro()
